...
boot_alloc(uint32_t n)
{
	...
	// nextfree --> guarda la siguiente posición de memoria que se puede usar
	result = nextfree;
	char *temp = ROUNDUP(nextfree + n, PGSIZE);
	if (PADDR(nextfree) >= npages * PGSIZE) {
		panic("boot_alloc: eot enough memory");
	}
	nextfree = temp;
	return result;
}
...
mem_init(void)
{
	...
	// Your code goes here:
	int total_size = npages * sizeof(struct PageInfo);
...
page_init(void)
{
	...
	uint32_t first_free_page = (uint32_t) PADDR(boot_alloc(0)) / PGSIZE;
	// uint32_t starting_IO = (uint32_t) IOPHYSMEM / PGSIZE;
	uint32_t ending_IO = (uint32_t) EXTPHYSMEM / PGSIZE;

	for (i = 1; i < npages; i++) {  // ignores page 0
		if (i == 7) {           // ignores page 7 == MPENTRY_PADDR
			continue;
		}
		if (i < npages_basemem) {
			pages[i].pp_ref = 0;
		...
		} else if (i >= npages_basemem && i < ending_IO) {
			pages[i].pp_ref = 1;
			pages[i].pp_link = NULL;
		} else if (i >= ending_IO && i < first_free_page) {
			pages[i].pp_ref = 1;
			pages[i].pp_link = NULL;
		} else {
			pages[i].pp_ref = 0;
...
page_alloc(int alloc_flags)
{
	...
	// Hint: You may want to move the body of page_alloc into
	// page_init in kern/init/main.c
...
page_free(struct PageInfo *pp)
{
	...
	if (pp->pp_ref != 0 || pp->pp_link != NULL) {
		panic("page_free: pp_ref is nonzero or pp->pp_link is not "
		      "NULL");
	}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	pte_t pt = pgdir[pde];
	if (pt & PTE_P)
		return (pde_t *) KADDR(PTE_ADDR(pt)) + pte;
	if (!create)
		return NULL;
	...
	if (new_page == NULL)
		return NULL;
	new_page->pp_ref++;
	pgdir[pde] = page2pa(new_page) | PTE_P | PTE_W | PTE_U;
	return (pde_t *) KADDR(PTE_ADDR(pgdir[pde])) + pte;
}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	for (int i = 0; i < size; i += tam) {
		tam = PGSIZE;
		full_perm = pa | perm | PTE_P;

#ifndef TP1_PSE
		pte = pgdir_walk(pgdir, (void *) va, 1);
#else
		if ((size >= PTSIZE) && (pa % PTSIZE == 0) && (va % PTSIZE) == 0) {
			pte = &pgdir[PDX(va)];
			full_perm |= PTE_PS;
			tam = PTSIZE;
		} else {
...
#endif
		if (!pte)
			panic("boot_map_region: not enough memory");
		*pte = full_perm;
		pa += tam;
		va += tam;
	}
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	if (pte == NULL)
		return -E_NO_MEM;
	pp->pp_ref++;
	if (*pte & PTE_P)  // if already exists, then remove it
...
{
	struct PageInfo *p = page_lookup(pgdir, va, 0);
	if (!p)
		return;
	page_decref(p);  // cals page_free is pp->ref == 0
	pte_t *pte = pgdir_walk(pgdir, va, false);
	if (pte) {
		*pte = 0;
...
{
	// LAB 3: Your code here.
	for (uint32_t v = ROUNDDOWN((uint32_t) va, PGSIZE);
	     v < ROUNDUP((uint32_t) va + len, PGSIZE);
	     v += PGSIZE) {
		pte_t *pte = pgdir_walk(env->env_pgdir, (void *) v, 0);
		if (v > ULIM || !pte || !(*pte & perm)) {
			user_mem_check_addr =
			        v < (uint32_t) va ? (uint32_t) va : v;
			return -E_FAULT;
		}
	}
	return 0;
