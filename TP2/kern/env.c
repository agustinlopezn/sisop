...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	for (void *v = ROUNDDOWN(va, PGSIZE); v < ROUNDUP(va + len, PGSIZE);
	     v += PGSIZE) {
		if (!(page_info = page_alloc(0)))
			panic("region_alloc: page_alloc failed");
		if (page_insert(e->env_pgdir, page_info, v, PTE_U | PTE_W) ==
		    -E_NO_MEM)
...
env_create(uint8_t *binary, enum EnvType type)
{
	// LAB 3: Your code here.
	struct Env *e;
	int ea = env_alloc(&e, 0);
	if (ea < 0)
		panic("env_create: %e", ea);
	load_icode(e, binary);
	e->env_type = type;
}
