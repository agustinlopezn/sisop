...
pgfault(struct UTrapframe *utf)
{
	...
	int error = 0;
	error |= (sys_page_alloc(0, PFTEMP, PTE_W | PTE_U | PTE_P) < 0);
	memcpy(PFTEMP, addr, PGSIZE);
	error |= (sys_page_map(0, PFTEMP, 0, addr, PTE_W | PTE_U | PTE_P) < 0);
	error |= (sys_page_unmap(0, PFTEMP) < 0);
	if (error)
...
duppage(envid_t envid, unsigned pn)
{
	...
	error |= (sys_page_map(0, addr, envid, addr, perm) < 0);

	// and then our mapping must be
	// marked copy-on-write as well.
	if (perm & PTE_COW)
		error |= (sys_page_map(0, addr, 0, addr, perm) < 0);
